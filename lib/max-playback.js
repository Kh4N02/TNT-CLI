...
const { quoteCmdArg, sanitizeSaveName, formatMaxManifestUrl } = require('./tnt-cmd-util');
const {
  downloadProxyUrl,
...
async function fetchMpdText(manifestUrl) {
  const headers = {
    ...
    Origin: SITE_ORIGIN,
    Referer: `${SITE_ORIGIN}/`,
    ...maxCdnDownloadHeaders(manifestUrl),
  };
...
function buildDownloadArgv(manifestUrl, keys, title, session, { proxyUrl = null } = {}) {
  ...
  args.push('-sv', process.env.TNT_VIDEO_SELECT || 'best');
...
async function resolveMaxPlayback(session, editId, { onStatus, displayTitle = null } = {}) {
  ...
  const manifestFromApi = pickManifestUrl(playback);
  const manifestUrl = formatMaxManifestUrl(manifestFromApi);
  if (/\.(?:emea|amer)-free\./i.test(manifestFromApi) && !playback?.drm) {
    throw new Error(
  ...
  if (manifestUrl !== manifestFromApi) {
    logs.push('Manifest upgraded to main.mpd (full 1080p ladder)');
  }
  logs.push(`Manifest: ${manifestUrl.slice(0, 120)}...`);
